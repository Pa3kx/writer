...
async def init_tables(pool: asyncpg.Pool) -> None:
    ...
    logger.debug("Table for measurements created")
        


async def store_measurements(
    pool: asyncpg.Pool, measurements: list[Measurement]
) -> None:
    ...
    async with pool.acquire() as conn:
        try:
            async with conn.transaction():
                ...
                logger.debug(f"Stored measurements: {len(measurements)}")
        except Exception as e:
            logger.error(f"Error storing measurements, reverting: {e}")
            await conn.rollback()
            raise

async def get_measurements(
    pool: asyncpg.Pool, kinds: list[str], from_time: int, to_time: int
    ...
    async with pool.acquire() as conn:
        ...
        for row in rows:
            ...
            measurement = Measurement(
                kind=row["kind"], time=row["time"], value=row["value"]
            )
            if row["kind"] not in result:
                result[row["kind"]] = []
            result[row["kind"]].append(measurement)

        logger.debug(f"Retrieved {len(result)} measurements")
        return result
